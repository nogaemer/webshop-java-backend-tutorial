...
public class ProductRepository {
    ...
    public ArrayList<ProductResponse> findAll(List<String> tags){
        ArrayList<ProductResponse> products = new ArrayList<>(Arrays.asList(
                new ProductResponse(
                        "1",
                        "AMD Ryzen 9 7950X",
                        "Der neue Prozessor",
        ...
        if (tags == null) {
            ...
            ArrayList<ProductResponse> filtered = products;
            tags = ListUtils.toLowerCase(tags);

            for (String tag : tags) {
                filtered.removeIf(product -> !ListUtils.toLowerCase(product.getTags()).contains(tag));
            }
            return filtered;
        }
    }
